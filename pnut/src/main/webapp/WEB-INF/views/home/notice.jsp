...
<c:if test="${not empty list }">
	...
	<div class="notice_container" onclick="link_to('${notice.notice_no }')">
		...
		<c:if test="${notice.n_type == 'renut' }">
				<c:out value="${notice.active_nn } @${notice.active}"/>님이 리넛하셨습니다<br>
				<c:if test="${notice.read == 'n' }"><span class="badge bg-info text-dark">new</span></c:if>				
		</c:if>
		<c:if test="${notice.n_type == 'reply' }">
				<c:out value="${notice.active_nn } @${notice.active}"/>님이 댓글을 다셨습니다<br>
				<c:if test="${notice.read == 'n' }"><span class="badge bg-info text-dark">new</span></c:if>				
		</c:if>
		<c:if test="${notice.n_type == 'follow' }">
				<c:out value="${notice.active_nn } @${notice.active}"/>님이 팔로우하셨습니다<br>
				<c:if test="${notice.read == 'n' }"><span class="badge bg-info text-dark">new</span></c:if>
		</c:if>
				<small>
				<c:choose>
					<c:when test="${(now - notice.regdate.time) > (1000*60*60*24*7*4)}">
						<fmt:formatDate value="${notice.regdate }" pattern="yyyy년 MM월 dd일"/>
					</c:when>
					<c:when test="${(now - notice.regdate.time) <= (1000*60*60*24*7*4) and (now - notice.regdate.time) > (1000*60*60*24*7)}">
						<fmt:parseNumber value="${(now - notice.regdate.time) / (1000*60*60*24*7) }" integerOnly="true"/>주 전
					</c:when>
					<c:when test="${(now - notice.regdate.time) <= (1000*60*60*24*7) and (now - notice.regdate.time) > (1000*60*60*24)}">
						<fmt:parseNumber value="${(now - notice.regdate.time) / (1000*60*60*24) }" integerOnly="true"/>일 전
					</c:when>
					<c:when test="${(now - notice.regdate.time) <= (1000*60*60*24) and (now - notice.regdate.time) > (1000*60*60)}">
						<fmt:parseNumber value="${(now - notice.regdate.time) / (1000*60*60) }" integerOnly="true"/>시간 전
					</c:when>
					<c:when test="${(now - notice.regdate.time) <= (1000*60*60) and (now - notice.regdate.time) > (1000*60)}">
						<fmt:parseNumber value="${(now - notice.regdate.time) / (1000*60) }" integerOnly="true"/>분 전
					</c:when>
					<c:otherwise>
						1분 이내
					</c:otherwise>
